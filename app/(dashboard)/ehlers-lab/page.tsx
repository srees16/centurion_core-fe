...
import { useEhlersChapters, useEhlersRun } from "@/hooks/use-ehlers";
import { EHLERS_CATEGORIES, DEFAULT_EHLERS_TICKERS } from "@/lib/constants";
import { format } from "date-fns";
import { Play, Activity, CalendarDays, Tag, Upload, ChevronDown, ChevronRight, X, History } from "lucide-react";
import { cn } from "@/lib/utils";

type TickerMode = "default" | "manual" | "csv";

export default function EhlersLabPage() {
  const chaptersQ = useEhlersChapters();
  const { run, isRunning, progress, error, abort, isAborted } = useEhlersRun();
  ...
  const grouped = EHLERS_CATEGORIES.map((cat) => ({
    category: cat,
  ...
  const getActiveTickers = (): string[] => {
    if (tickerMode === "default") return DEFAULT_EHLERS_TICKERS;
    if (tickerMode === "csv") return csvTickers;
    return manualTickers.split(",").map((t) => t.trim().toUpperCase()).filter(Boolean);
  };
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const parsed = text.split(/[\n,]/).map((t) => t.trim().toUpperCase()).filter((t) => t && t !== "TICKER" && /^[A-Z]{1,10}$/.test(t));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold">Ehlers DSP Lab</h2>
        <span className="text-xs text-muted-foreground">Cybernetic Analysis & Rocket Science for Traders</span>
        <a href="/ehlers-lab/history" className="ml-auto flex items-center gap-1.5 text-base font-semibold text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="content-panel p-4 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {tickerMode === "default" && (
              <div className="text-xs text-muted-foreground bg-muted/50 rounded px-2 py-1.5">
                {DEFAULT_EHLERS_TICKERS.join(", ")}
              </div>
            )}
            {tickerMode === "manual" && (
              <Input placeholder="MSFT, GOOG, NVDA, AMD" value={manualTickers} onChange={(e) => setManualTickers(e.target.value)} className="text-xs" />
            )}
            {tickerMode === "csv" && (
              <div className="space-y-1">
                <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer border border-dashed rounded px-2 py-1.5 hover:bg-muted/50">
                  ...
                  {csvTickers.length > 0 ? `${csvTickers.length} tickers loaded` : "Upload CSV"}
              ...
              <div>
                ...
                <Popover open={startOpen} onOpenChange={setStartOpen}>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" captionLayout="dropdown" selected={dateStart} onSelect={(d) => { if (d) { setDateStart(d); setStartOpen(false); } }} defaultMonth={dateStart} startMonth={new Date(2020, 0)} endMonth={new Date(2030, 11)} />
                  </PopoverContent>
                </Popover>
              </div>
              <div>
                ...
                <Popover open={endOpen} onOpenChange={setEndOpen}>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" captionLayout="dropdown" selected={dateEnd} onSelect={(d) => { if (d) { setDateEnd(d); setEndOpen(false); } }} defaultMonth={dateEnd} startMonth={new Date(2020, 0)} endMonth={new Date(2030, 11)} />
                  </PopoverContent>
                </Popover>
              </div>
            </div>
            <p className="text-[0.65rem] text-muted-foreground">
              Applies to chapters that fetch live market data.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              {isRunning && (
                <Button variant="destructive" onClick={abort}><X className="mr-1 h-4 w-4" /> Cancel</Button>
              )}
            </div>
                  ...
                  <div key={g.category}>
                    ...
                    {g.items.map((ch) => (
                      <div key={ch.key} className="flex items-start gap-2 py-0.5">
                        <Checkbox checked={selected.includes(ch.key)} onCheckedChange={(v) => toggleChapter(ch.key, v === true)} className="mt-0.5" />
                        <div className="flex-1 min-w-0">
                          <Label className="text-sm font-normal">{ch.title}</Label>
                          {ch.description && <p className="text-[0.65rem] text-muted-foreground leading-tight">{ch.description}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="md:col-span-3 space-y-4">
          ...
          {isAborted && progress && (
            <div className="content-panel p-4 border-destructive">
              <p className="text-sm font-medium text-destructive">Aborted — {progress.completed}/{progress.total} chapters completed</p>
            </div>
          )}
            ...
            Object.entries(progress.chapters).map(([key, ch]) => {
              ...
              return (
                <div key={key} className="content-panel p-4 space-y-2">
                  ...
                  >
                    {canCollapse && (isCollapsed ? <ChevronRight className="h-4 w-4 text-muted-foreground shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />)}
