...
export default function VinceLabPage() {
  const chaptersQ = useVinceChapters();
  const { run, isRunning, progress, error, abort, isAborted } = useVinceRun();
  ...
  const grouped = VINCE_CATEGORIES.map((cat) => ({
    category: cat,
  ...
  const getActiveTickers = (): string[] => {
    if (tickerMode === "default") return DEFAULT_VINCE_TICKERS;
