...
export default function AronsonLabPage() {
  const chaptersQ = useAronsonChapters();
  const { run, isRunning, progress, error, abort, isAborted } = useAronsonRun();
  ...
  const grouped = ARONSON_CATEGORIES.map((cat) => ({
    category: cat,
  ...
  const getActiveTickers = (): string[] => {
    if (tickerMode === "default") return DEFAULT_ARONSON_TICKERS;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <a href="/aronson-lab/history" className="ml-auto flex items-center gap-1.5 text-base font-semibold text-muted-foreground hover:text-foreground transition-colors">
