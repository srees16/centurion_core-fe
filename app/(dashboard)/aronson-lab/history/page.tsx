...
import { useAronsonHistory } from "@/hooks/use-aronson";
import { ShieldCheck } from "lucide-react";

export default function AronsonHistoryPage() {
  const historyQ = useAronsonHistory();

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <ShieldCheck className="h-5 w-5 text-amber-500" />
        <h2 className="text-lg font-semibold">Aronson Lab — Run History</h2>
      </div>
      ...
      {historyQ.error && (
        <p className="text-sm text-destructive">
          {historyQ.error instanceof Error ? historyQ.error.message : String(historyQ.error)}
        </p>
      )}
    </div>
  );
