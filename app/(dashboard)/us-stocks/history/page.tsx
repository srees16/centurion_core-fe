...
export default function USHistoryPage() {
  ...
  const signalQ = useSignalHistory("US");
  const backtestQ = useBacktestHistory("US");
                  ...
                  <tbody>
                    {signalQ.data.data.map((r) => (
                      <tr key={r.id} className="border-b">
                        ...
                        <td className="py-2 pr-3">{r.rsi.toFixed(1)}</td>
                        <td className="py-2 text-xs truncate max-w-[200px]">{r.source}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="content-panel p-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {backtestQ.data.data.map((r) => (
                      <tr key={r.id} className="border-b">
                        <td className="py-2 pr-3 text-xs">{new Date(r.created_at).toLocaleString()}</td>
                        <td className="py-2 pr-3">{r.strategy_name}</td>
                        <td className="py-2 pr-3 font-mono text-xs">{r.tickers.join(", ")}</td>
                        <td className={`py-2 pr-3 ${r.total_return >= 0 ? "pnl-positive" : "pnl-negative"}`}>
                        ...
                        <td className="py-2 pr-3">{formatPct(r.win_rate)}</td>
